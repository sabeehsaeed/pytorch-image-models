...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            roc = roc_auc_score(target.cpu().detach().numpy(), torch.argmax(output, dim=1).cpu().detach().numpy(), average='micro', zero_division=1)
            ...
            top1_f1.update(f1, output.size(0))
            top1_roc.update(roc, output.size(0))
